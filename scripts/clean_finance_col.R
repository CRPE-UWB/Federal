...
rename_col <- function(df) {
  ...
  names(df)[names(df) == "C12"] <- "STATEREV_CAPITALOUTLAY-DEBTSERVICES"
  ...
  names(df)[names(df) == "TOTALEXP"] <- "EXPEND_TOTAL"
  ...
  names(df)[names(df) == "Z35"] <- "TEACHERSALARIES_REGED"
  names(df)[names(df) == "Z36"] <- "TEACHERSALARIES_SPED"
  names(df)[names(df) == "Z37"] <- "TEACHERSALARIES_VOCATIONALED"
  names(df)[names(df) == "Z38"] <- "TEACHERSALARIES_OTEHRED"
  ...
  names(df)[names(df) == "V93"] <- "TEXTBOOXS"
  ...
  names(df)[names(df) == "X_61V"] <- "SHORTTERMDEBT_OUSTANDING_ATBEGOFFISCALYR"
  names(df)[names(df) == "X_66V"] <- "SHORTTERMDEBT_OUSTANDING_ATENDOFFISCALYR"
  names(df)[names(df) == "W01"] <- "ASSETS_SINKINGFUND"
  names(df)[names(df) == "W31"] <- "ASSETS_BONDFUND"
  names(df)[names(df) == "W61"] <- "ASSETS_OTHERFUNDS"
  names(df)[names(df) == "HR1"] <- "ARRA_REV-TITLEI"
...
reodrer_col <- function(df) {
  require(data.table)
  
  order <- c("LEAID", "FIPST", "FIPSCO", "CMSA", "NAME", "YEAR", "FALL_MEMBERSHIP", "TOTALREV",
             "FEDREV_TOTAL", "FEDREV_STATE_TITLEI", "FEDREV_STATE_IDEA",  "FEDREV_STATE_MATHSCIENCE-TEACHERQUALITY", "FEDREV_STATE_SAFE-DRUGFREE", "FEDREV_STATE_VOCATIONAL-TECHED",
             "FEDREV_STATE_BILINGUALED", "FEDREV_STATE_OTHER", "FEDREV_STATE_CHILDNUTRITIONACT", "FEDREV_NONSPECIFIED", "FEDREV_DIRECT_IMPACTAID", "FEDREV_DIRECT_INDIANED", "FEDREV_STATE_TITLEVPTA",
             "FEDREV_DIRECT_OTHERTOTALSTATEREV", "STATEREV_TOTAL", "STATEREV_GENFORMULA-ASSISTANCE", "STATEREV_STAFFIMPROV_PROGRAMS", "STATEREV_SPED_PROGRAMS", "STATEREV_COMPENSATORY-BASIC-SKILLSPROGRAM",
             "STATEREV_BILINGUALED", "STATEREV_GIFTED-TALENTED_PROGRAMS", "STATEREV_VOCATIONALED_PROGRAMS", "STATEREV_SCHOOLLUNCH", "STATEREV_CAPITALOUTLAY-DEBTSERVICES", "STATEREV_CAPITALOUTLAY-DEBTSERVICES",
             "STATEREV_OTHER", "STATEREV_NONSPECIFIED", "STATEREV_ONBEHALF_EMPLOYEE-BENEFITS", "STATEREV_ONBEHALF_NOTEMPLOYEE-BENEFITS", "LOCALREV_TOTAL", "LOCALREV_PARENTGOVCONRIBUTIONS",
             "LOCALREV_PROPERTY_TAXES", "LOCALREV_GENERALSALES_TAXES", "LOCALREV_PUBLICUTILITY_TAXES", "LOCALREV_INDIVIDUAL-CORPORATEINCOME_TAXES", "LOCALREV_ALLOTHER_TAXES", "LOCALREV_FROMOTHERSCHOOLSYSTEMS",
             "LOCALREV_FROMCITIES-COUNTIES", "LOCALREV_TUITIONFEESFROMPUPILS-PARENTS", "LOCALREV_TRANSPORTATIONFEESFROMPUPILS-PARENTS", "LOCALREV_SCHOOLLUNCH", "LOCALREV_TEXTBOXSALES-RENTALS", "LOCALREV_DISTRICTACTIVITY-RECEIPTS",
             "LOCALREV_STUDENTFEES-NONSPECIFIED", "LOCALREV_OTEHRSALES-SERVICES", "LOCALREV_RENTS-ROYALTIES", "LOCALREV_SALEOFPROPERTY", "LOCALREV_INTERESTEARNINGS", "LOCALREV_FINES-FORFEITS",
             "LOCALREV_PRIVATECONTRIBUTIONS", "LOCALREV_MISC", "LOCALREV_NCES_CENSUSBUREAUSTATEREV", "TOTALEXP", "CURRENTEXPEND_ELEMSECONDEXPEND_TOTAL", "CURRENTEXPEND_INSTRUCTION_TOTAL",
             "CURRENTEXPEND_INSTRUCTION", "PAYMENTS_TOPRIVATESCH", "PAYMENTS_TOCHARTERSCH", "CURRENTEXPEND_SUPPORTSERVICES_TOTAL", "CURRENTEXPEND_SUPPORTSERVICES_PUPILS", "CURRENTEXPEND_SUPPORTSERVICES_INSTRUCTIONALSTAFF",
             "CURRENTEXPEND_SUPPORTSERVICES_GENERALADMIN", "CURRENTEXPEND_SUPPORTSERVICES_SCHOOLADMIN", "CURRENTEXPEND_SUPPORTSERVICES_OP-MAINTENANCEOFPLANT", "CURRENTEXPEND_SUPPORTSERVICES_STUDENTTRANSPORTATION", "CURRENTEXPEND_SUPPORTSERVICES_BUSINESS-CENTRAL-OTHER", 
             "CURRENTEXPEND_SUPORTSERVICES_BUSINESS", "CURRENTEXPEND_SUPPORTSERVICES_CENTRAL", "CURRENTEXPEND_SUPPORTSERVICES_OTHER", "CURRENTEXPEND_SUPPORTSERVICES_NONSPECIFIED",
             "CURRENTEXPEND_OTHERELEMSECOND_TOTAL", "CURRENTEXPEND_FOODSERVICES", "CURRENTEXPEND_ENTERPRISEOPS", "CURRENTEXPEND_OTHER", "NONELEMSECONDEXPEND_TOTAL", "NONELEMSECONDEXPEND_COMMUNITYSERVICES",
             "NONELEMSECONDEXPEND_ADULTED", "NONELEMSECONDEXPEND_OTHER", "CAPITALOUTLAY_TOTAL", "CAPITALOUTLAY_CONSTRUCTION", "CAPITALOUTLAY_LAND-EXISTINGSTRUCTURES", "CAPITALOUTLAY_INSTRUCTIONALEQUIPMENT",
             "CAPITALOUTLAY_OTHEREQUIPMENT", "CAPITALOUTLAY_NONSPECIFEQUIPMENT", "PAYMENTS_TOSTATEGOV", "PAYMENTS_TOLOCALGOV", "PAYMENTS_TOOTHERSCHOOLSYSTEMS", "INTERESTONDEBT",
             "SALARIES_TOTAL", "SALARIES_INSTRUCTION", "TEACHERSALARIES_REGED", "TEACHERSALARIES_SPED", "TEACHERSALARIES_VOCATIONALED", "TEACHERSALARIES_OTEHRED",
             "SALARIES_SUPPORTSERVICES_PUPILS", "SALARIES_SUPPORTSERVICES_INSTRUCTIONALSTAFF", "SALARIES_SUPPORTSERVICES_GENERALADMIN", "SALARIES_SUPPORTSERVICES_SCHOOLADMIN", "SALARIES_SUPPORTSERVICES_OP-MAINTENANCEOFPLANT", "SALARIES_SUPPORTSERVICES_STUDTRANSPORTATION",
             "SALARIES_SUPPORTSERVICES_BUSINESS-CENTRAL_OTHERS", "SALARIES_SUPPORTSERVICES_BUSINESS", "SALARIES_SUPPORTSERVICES_CENTRAL", "SALARIES_SUPPORTSERVICES_OTHER", "SALARIES_FOODSERVICES", "EMPBENEFITS_TOTAL", "EMPBENEFITS_INSTRUCTION", "EMPBENEFITS_SUPPORTSERVICES_PUPILS", "EMPBENEFITS_SUPPORTSERVICES_INSTRUCTIONALSTAFF", "EMPBENEFITS_SUPPORTSERVICES_BUSINESS-CENTRAL_OTHERS",
             "EMPBENEFITS_SUPPORTSERVICES_BUSINESS", "EMPBENEFITS_SUPPORTSERVICES_CENTRAL", "EMPBENEFITS_SUPPORTSERVICES_OTHER", "EMPBENEFITS_FOODSERVICES",
             "EMPBENEFITS_SUPPORTSERVICES_GENERALADMIN", "EMPBENEFITS_SUPPORTSERVICES_SCHOOLADMIN", "EMPBENEFITS_SUPPORTSERVICES_OP-MAINTENANCEOFPLANT", "EMPBENEFITS_SUPPORTSERVICES_STUDTRANSPORTATION",
             "EMPBENEFITS_ENTERPRISEOPS", "TEXTBOOXS", "LONGTERMDEBT_OUTSTANDINGATBEGOFFISCALYR", "LONGTERMDEBT_ISSUEDDURINGFISCALYR", "LONGETERMDEBT_RETIREDDURINGFISCALYR", "LONGETERMDEBT_OUTSTANDINGATENDOFFISCALYR",
             "SHORTTERMDEBT_OUSTANDING_ATBEGOFFISCALYR", "SHORTTERMDEBT_OUSTANDING_ATENDOFFISCALYR", "ASSETS_SINKINGFUND", "ASSETS_BONDFUND", "ASSETS_OTHERFUNDS", "ARRA_REV-TITLEI", 
             "ARRA_CURRENTEXP", "ARRA_CAPITALOUTLAYEXP"
  )
    
  df <- setcolorder(x, order)
